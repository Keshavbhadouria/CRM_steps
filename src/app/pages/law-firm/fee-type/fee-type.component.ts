...
export class FeeTypeComponent implements OnInit {
  ...
  constructor(private _LawfirmFeetypeService: LawfirmFeeTypeService, private _messageService: MessageService, public modalService: NgbModal) { }
  ...
  getRecords() {
    this.showLoader();
    this._LawfirmFeetypeService.getAll(
      this.filterText,
      this.typeNameFilter,
      null,
  ...
  deleteProject(data): boolean {
    return this._LawfirmFeetypeService.delete(data.id).then(res => {
      if (res.success)
        return true;
      else
        return false;
