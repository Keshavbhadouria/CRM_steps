...
export class StaffAssignmentComponent implements OnInit {
  ...
  constructor(private _staffAssignmentService: StaffAssignmentService, private _messageService: MessageService, public modalService: NgbModal) { }
  ...
  ngOnInit() {
    this.breadCrumbItems = [{ label: 'Staff Assignment' }, { label: 'List', active: true }];
  ...
  getRecords() {
    this.showLoader();
    this._staffAssignmentService.getAll(
      this.filterText,
  ...
  deleteProject(data): boolean {
    return this._staffAssignmentService.delete(data.contactAssignParalegal.id).then(res => {
      if (res.success)
        return true;
      else
        return false;
