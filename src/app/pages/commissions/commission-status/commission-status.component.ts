...
import { CommissionStatusService } from 'src/app/core/services/commission-status.service';
...
export class CommissionStatusComponent implements OnInit {
  ...
  viewCommissionStatus: any;
  editCommissionStatus: any;
  loading = false;
  createLoader = false;
  ...
  tableSettings: ColumnSetting[] = [
    ...
    {
      primaryKey: 'commissionStatus.status',
      header: 'Status',
    }
  ];
  ...
  buttonSettings: ButtonSettings[] = [
    ...
    },
    {
      ...
      func: (obj) => {
        this.showLoader();
        this._commissionStatusService.getCommissionStatusForEdit(obj.commissionStatus.id).then(s => {
          this.hideLoader();
          this.editCommissionStatus = s.result.commissionStatus;
    ...
    },
    {
      title: 'Delete',
      func: (obj) => {
        this.onDelete(obj.commissionStatus.id);
      },
  ...
  constructor(private _messageService: MessageService,
    ...
    private _commissionStatusService: CommissionStatusService,
    private translate: TranslateService) { }

  ngOnInit(): void {
    this.breadCrumbItems = [{ label: 'Commissions' }, { label: 'CommissionStatuses', active: true }];
    this.paginationSettings.getRecords = () => this.getRecords();
    this.getRecords();
  }
  getRecords() {
    this.showLoader();
    this._commissionStatusService.getAll(
      this.filterText,
      this.statusFilter,
      null,
      ...
      this.hideLoader();
      if (res.success) {
        this.commissionStatusList = res.result.items;
        this.myData = [];
  ...
  openCreateModal(modal: any) {
    this.editCommissionStatus = null;
  ...
  onDelete(id) {
    ...
    swalWithBootstrapButtons
      ...
      .then(result => {
        if (result.value) {
          this.showLoader();
          this._commissionStatusService.delete(id)
            .then(res => {
              this.hideLoader();
              if (res.success) {
                this.getRecords();
