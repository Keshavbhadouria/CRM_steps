...
@Component({
  selector: 'app-contract-template-details',
  templateUrl: './contract-template-details.component.html',
  styleUrls: ['./contract-template-details.component.scss']
})
export class ContractTemplateDetailsComponent implements OnInit {

  constructor(private _contractTemplateDetailService: ContractTemplateDetailService, private _messageService: MessageService, public modalService: NgbModal, private _router: Router , private _activatedRoute : ActivatedRoute) {
   }
  ...
  tableSettings: ColumnSetting[] = [
    ...
    {
      primaryKey: 'contractTemplateDetail.title',
      header: 'Title',
    },
    {
      primaryKey: 'contractTemplateDetail.content',
      header: 'Content',
    },
    {
      primaryKey: 'contractTemplateDetail.sortIndex',
      header: 'SortIndex',
    },
    {
      primaryKey: 'contractTemplateDetail.active',
      header: 'Active',
    },
  ];
  ...
  getRecords() {
    this.showLoader();
    this._contractTemplateDetailService.getAll(
      this.filterText,
      this.titleFilter,
      this.contentFilter,
  ...
  onDelete(dataObj) {
    ...
    swalWithBootstrapButtons
      ...
      .then(result => {
        if (result.value) {
          if (this.deleteProject(dataObj.contractTemplateDetail)) {
  ...
  deleteProject(data): boolean {
    return this._contractTemplateDetailService.delete(data.id).then(res => {
      if (res.success)
        return true;
      else
        return false;
