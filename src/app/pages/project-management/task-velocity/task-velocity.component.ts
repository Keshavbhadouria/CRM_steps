...
@Component({
  selector: 'app-task-velocity',
  templateUrl: './task-velocity.component.html',
  styleUrls: ['./task-velocity.component.scss']
})
export class TaskVelocityComponent implements OnInit {
 ...
 ngOnInit(): void {
   this.breadCrumbItems = [{ label: 'Tasks' }, { label: 'TaskVelocities', active: true }];
 ...
 getRecords() {
   this.showLoader();
   this._taskVelocityService.getAll(
     this.filterText,
     this.statusFilter,
     null,
 ...
 deleteProject(data): boolean {
   return this._taskVelocityService.delete(data.id).then(res => {
     if (res.success)
       return true;
     else
       return false;
