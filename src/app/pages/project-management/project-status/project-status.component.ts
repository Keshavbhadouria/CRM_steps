...
export class ProjectStatusComponent implements OnInit {
  ...
  constructor(private _projectStatusService: ProjectStatusService, private _messageService: MessageService, public modalService: NgbModal) { }
  ...
  ngOnInit() {
    this.breadCrumbItems = [{ label: 'Projects' }, { label: 'Project Status List', active: true }];
  ...
  getRecords() {
    this.showLoader();
    this._projectStatusService.getAll(
      this.filterText,
      this.statusFilter,
      null,
  ...
  deleteProject(data): boolean {
    return this._projectStatusService.delete(data.id).then(res => {
      if (res.success)
        return true;
      else
        return false;
