...
@Component({
  selector: 'app-sprint-types',
  templateUrl: './sprint-types.component.html',
  styleUrls: ['./sprint-types.component.scss']
})
export class SprintTypesComponent implements OnInit {



  constructor(private _projectSprintTypesService: SprintTypesService, private _messageService: MessageService, public modalService: NgbModal) { }
  ...
  ngOnInit() {
    this.breadCrumbItems = [{ label: 'PM' }, { label: 'Sprint Types List', active: true }];
  ...
  getRecords() {
    this.showLoader();
    this._projectSprintTypesService.getAll(
      this.filterText,
      this.TypesFilter,
      null,
  ...
  onDelete(pmProjectTypes) {
    ...
    swalWithBootstrapButtons
      ...
      .then(result => {
        if (result.value) {
          if (this.deleteProject(pmProjectTypes)) {
            swalWithBootstrapButtons.fire(
              'Deleted!',
              'Your file has been deleted.',
              'success'
            );
            this.myData = this.myData.filter(obj => { return obj !== pmProjectTypes });
          } else {
            this._messageService.showServerSideError();
          }
        }
  ...
  deleteProject(data): boolean {
    return this._projectSprintTypesService.delete(data.id).then(res => {
      if (res.success)
        return true;
      else
        return false;
