...
export class TaskTypesComponent implements OnInit {
  ...
  constructor(private _taskTypesService: PMTaskTypesServiceProxy,
 ...
 ngOnInit(): void {
   this.breadCrumbItems = [{ label: 'Tasks' }, { label: 'TaskTypes', active: true }];
 ...
 deleteProject(data): boolean {
   return this._taskTypesService.delete(data.id).then(res => {
     if (res.success)
       return true;
     else
       return false;
