...
import { ExpenseTypeService } from 'src/app/core/services/expense-type.service';
...
export class ExpenseTypesComponent implements OnInit {
  ...
  tableSettings: ColumnSetting[] = [
    ...
    {
      primaryKey: 'serviceExpenseType.spentType',
      header: 'SpentType',
    }
  ];
  ...
  buttonSettings: ButtonSettings[] = [
    ...
    },
    {
      ...
      func: (obj) => {
        this.showLoader();
        this._expenseTypeService.getServiceExpenseTypeForEdit(obj.serviceExpenseType.id).then(s => {
          this.hideLoader();
          this.editExpenseType = s.result.serviceExpenseType;
    ...
    },
    {
      title: 'Delete',
      func: (obj) => {
        this.onDelete(obj.serviceExpenseType.id);
      },
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [{ label: 'Expenses' }, { label: 'ExpenseTypes', active: true }];
  ...
  getRecords() {
    ...
    this._expenseTypeService.getAll(
      ...
      this.hideLoader();
      if (res.success) {
        this.expenseTypeList = res.result.items;
        this.myData = [];
  ...
  onDelete(id) {
    ...
    swalWithBootstrapButtons
      ...
      .then(result => {
        if (result.value) {
          this.showLoader();
          this._expenseTypeService.delete(id)
            .then(res => {
              this.hideLoader();
              if (res.success) {
                this.getRecords();
