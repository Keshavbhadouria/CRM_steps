...
import { RefundPaymentReasonService } from 'src/app/core/services/refund-payment-reason.service';
import { environment } from 'src/environments/environment';
import Swal from 'sweetalert2';
@Component({
  selector: 'app-refund-payments-reasons',
  templateUrl: './refund-payments-reasons.component.html',
  styleUrls: ['./refund-payments-reasons.component.scss']
})
export class RefundPaymentsReasonsComponent implements OnInit {



  constructor(private _RefundPaymentReasonService: RefundPaymentReasonService, private _messageService: MessageService, public modalService: NgbModal) { }
  ...
  ngOnInit() {
    this.breadCrumbItems = [{ label: 'PM' }, { label: 'FeeType', active: true }];
  ...
  getRecords() {
    this.showLoader();
    this._RefundPaymentReasonService.getAll(
      this.filterText,
      null,
  ...
  deleteProject(data): boolean {
    return this._RefundPaymentReasonService.delete(data.id).then(res => {
      if (res.success)
        return true;
      else
        return false;
