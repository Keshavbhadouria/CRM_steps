...
export class StoryByStagesNumberChart extends DashboardChartBase {
  chartData: any[] = [];
  ...
  reload(sprintId) {
    this.showLoading();
    this._dashboardService.getStoryStages(sprintId).then(data => {
...
@Component({
  selector: 'app-widget-story-stages',
  templateUrl: './widget-story-stages.component.html',
  styleUrls: ['./widget-story-stages.component.scss']
})
export class WidgetStoryStagesComponent implements OnInit {

  storiesByStageBarChart: StoryByStagesNumberChart;
  cardColor = "#232837";
  constructor(private _dashboardService: TenantDashboardService,
    public _zone: NgZone) {
    this.storiesByStageBarChart = new StoryByStagesNumberChart(this._dashboardService);
  }
